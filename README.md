...
8. If SmartScreen appears, choose More info, then Run anyway if you trust the source.
...
3. Connect the required sensor or control input if your setup uses one.
4. Watch the power status and elevator state on the screen.
5. When a power cut is detected, the rescue process starts.
6. The system selects the nearest floor.
7. The cabin moves to that floor using the control logic.
8. Passengers can exit once the cabin stops at the safe level.
...
### The app window does not show
- Close the app from Task Manager
- Open it again
- Restart Windows if needed

### The rescue process does not start
...
4. Open the app file
5. Let the system detect the power cut
6. Let the rescue logic move the cabin to the nearest floor